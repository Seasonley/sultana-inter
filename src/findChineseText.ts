 ...
 * @param code
 */
function findTextInTs(code: string, fPath: string, conf: any) {
  const ext = fPath.slice(-3).replace('.', '').toUpperCase() as 'JS'|'JSX'|'TS'|'TSX';
  const matches = [];
      ...
      case ts.SyntaxKind.JsxElement: {
        ...
        children.forEach(child => {
          if (child.kind === ts.SyntaxKind.JsxText) {
            ...
            const noCommentText = removeFileComment(text, fPath);

            if (noCommentText.match(DOUBLE_BYTE_REGEX)) {
              const start = child.getStart();
              const end = child.getEnd();
              const { trimStart, trimEnd } = trimWhiteSpace(code, start, end);
 ...
 * @param code
 */
function findTextInHtml(code,fPath,conf:any) {
  const matches = [];
 ...
 * @param code
 */
export default function findChineseText(code: string, fPath:string,conf:any) {
