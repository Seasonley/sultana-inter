...
function saveCache(projectRoot, map) {
    const cachePath = getCachePath(projectRoot);
...
class LlmKeyer {
    ...
    async mapKeys(entries) {
        // Load existing cache
        const cache = loadCache(this.projectRoot) || new Map();
        ...
        for (let i = 0; i < batches.length; i++) {
            ...
            try {
                ...
                logger_1.logger.log('warn', 'llm-keyer', `Batch ${i + 1}/${batches.length} failed: ${err}. Falling back to deterministic keys for this batch.`);
...
exports.LlmKeyer = LlmKeyer;
/** Exposed for testing: whether credentials exist. */
LlmKeyer.hasCredentials = hasCredentials;
//# sourceMappingURL=keyer-llm.js.map