...
exports.extractAngular = extractAngular;
/**
 * Angular 模板提取器
 *
 ...
 */
function deriveBasePath(relPath) {
 ...
 * @returns 提取到的内联模板数组
 */
function extractInlineTemplatesFromTS(tsContent) {
    const results = [];
    const sourceFile = ts.createSourceFile('component.ts', tsContent, ts.ScriptTarget.Latest, true);
    /**
    ...
    function visit(node) {
        ...
        if (ts.isClassDeclaration(node)) {
            ...
            const allModifiers = node.modifiers;
            if (allModifiers) {
 ...
 */
function extractTemplateFromDecorator(decorator, tsContent, results) {
    ...
    for (const property of objectLiteral.properties) {
        ...
        const propName = ts.isIdentifier(name) ? name.text :
 ...
 * 收集所有包含中文字符的文本片段。
 */
function visitNodes(nodes, code, matches) {
    for (const node of nodes) {
        ...
        if (value && typeof value === 'string') {
            ...
            if (DOUBLE_BYTE_REGEX.test(decoded)) {
                ...
                const valueSpan = node.valueSpan ||
                    node.sourceSpan;
                if (!valueSpan)
                    continue;
                const { start: { offset: startOffset }, end: { offset: endOffset }, } = valueSpan;
 ...
 */
function parseTemplateAndExtract(templateContent, filePath, relPath, callSiteType) {
    const entries = [];
    ...
    try {
        ...
        logger_1.logger.log('warn', 'angular-extract', `parseTemplate 失败: ${relPath}`, {
            error: String(err),
        });
        return entries;
    }
 ...
 * @returns         ExtractedEntry 数组
 */
function extractAngular(filePath, relPath, content) {
    const ext = relPath.split('.').pop()?.toLowerCase();
    // ── .html 文件：直接解析 HTML 模板 ──
    if (ext === 'html') {
        return parseTemplateAndExtract(content, filePath, relPath, 'angular-html');
    }
    ...
    if (ext === 'ts' || ext === 'tsx') {
        ...
        for (const tpl of inlineTemplates) {
            const tplEntries = parseTemplateAndExtract(tpl.template, filePath, relPath, 'angular-inline');
